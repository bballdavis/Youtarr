...
| `external_requests` | `ExternalRequest` | Per-API-key external request history and administrator review state. Active video requests use a nullable unique dedupe key; rejection and terminal failures clear it so a later retry can be created. `job_id` links an accepted request to its stable UUID downloader job. Optional client idempotency values are stored only as SHA-256 digests. |
